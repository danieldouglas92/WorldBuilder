...
#include "world_builder/features/subducting_plate_models/composition/coh_water_content.h"
...
namespace WorldBuilder
{
  ...
  namespace Features
  {
    namespace SubductingPlateModels
    {
      ...
      {
        COHWaterContent::COHWaterContent(WorldBuilder::World *world_)
          :
          min_depth(NaN::DSNAN),
          max_depth(NaN::DSNAN),
          density(NaN::DSNAN)
        {
          this->world = world_;
          this->name = "COH  water content";
        }

        COHWaterContent::~COHWaterContent()
          = default;

        void
        COHWaterContent::declare_entries(Parameters &prm, const std::string & /*unused*/)
        {
          // Document plugin and require entries if needed.
          // Add compositions to the required parameters.
          prm.declare_entry("", Types::Object({"compositions"}),
                            "COHWaterContent compositional model. Sets bound water content as a compositional field. The returned "
        ...
        void
        COHWaterContent::parse_entries(Parameters &prm)
        {
        ...
        double
        COHWaterContent::calculate_water_content(double pressure,
        ...
        double
        COHWaterContent::get_composition(const Point<3> &position_in_cartesian_coordinates,
                                          const double depth,
        ...
        WB_REGISTER_FEATURE_SUBDUCTING_PLATE_COMPOSITION_MODEL(COHWaterContent, coh water content)
      } // namespace Composition
    } // namespace SubductingPlateModels
  } // namespace Features
} // namespace WorldBuilder
